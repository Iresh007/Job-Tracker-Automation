...
router.patch("/applications/:id", async (req, res): Promise<void> => {
  ...
  // Drizzle timestamp columns require Date objects; convert interviewAt string → Date
  const { interviewAt, ...rest } = parsed.data;
  const updatePayload: Record<string, unknown> = { ...rest };
  if (interviewAt !== undefined) {
    updatePayload.interviewAt = interviewAt ? new Date(interviewAt) : null;
  }

  const [app] = await db
    .update(applicationsTable)
    .set(updatePayload as unknown as typeof applicationsTable.$inferInsert)
